...
## Extra dimension = Renormalization flow

Holography normally connects two theories which are both quantum-mechanical, a 3D quantum gravity and a 2D quantum field theory. However, and that's definitely surprising, the classical limit for one is not the classical limit for another. In other words, their respective $$ \hbar $$s are not the same.

In particular, let's consider taking the classical limit on the gravitational side. For a reason I won't go into much detail now, making a gravitational theory classical is the same as making it weakly coupled. The Planck scale is *both* the scale at which gravity becomes quantum and that at which it becomes strong; to us at much lower energy scales it becomes both very classical and very weak, and thus general relativity plus a very small big $$ G $$. So, that sounds like an interesting corner to study, but what happens to the reflection in the holographic mirror? It turns out that making the bulk classical/weak does not make the boundary either of those things. It makes it a **beautiful rainbow**. More seriously, the 2D theory is often something similar to quantum chromodynamics, involving a bunch of """"quarks"""" which can have a certain number $$ N $$ of colours, and interacting through the exchange of "gluons". When we make the 3D side classical + weak, the 2D side remains very much quantum, but the number of colours grows very large.

Putting aside the question of colours, the equality of a quantum and a classical theory is an undeniably fascinating beast. That's roughly because a classical system plays by strict rules, and a quantum one doesn't. In classical mechanics, a system has a unique configuration which evolves in time following a precise rule known as the **equation of motion**. Actually, in the case of something like gravity, where time and space are on equal footing (thank you Einstein) the evolution of quantities is given both in time but in a certain sense also in space. Think of the electrostatic field from a charge, or even better the gravitational field from a point mass. Since there is no action at a distance, the mass/charge generates the field only in its immediate vicinity; then to move outwards and find the value of the field at some distance you need to solve an equation encoding how each bit of field affects its neighbouring bits. That equation is a spatial equation of motion, and solving it gives you the celebrated $$ 1 / r^2 $$ falloff of the field.

A quantum system has a huge freedom, in comparison. It is not forced to follow the equation of motion, but rather all possible ways things could go from an initial to a final situation are considered and give a tiny contribution for the probability of the entire transition to happen. The history that satisfies the equations of motion only happens to be the one that gives the biggest contribution, but the credit for the final result is to be found in a spread out region of absurd histories around it that do not. In this sense, a quantum system is said to be **off-shell**, while its virtuous classical counterpart is on-shell.

How can two theories with such a gigantic discrepancy in freedom be the same? The holographic solution is that this extra freedom of the 2D theory is balanced perfectly by the **additional dimension of space** in the 3D theory.
